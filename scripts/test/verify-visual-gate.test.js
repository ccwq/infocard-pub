...
function exceptionFixture(count = 3, options = {}) {
  ...
  const rounds = options.rounds === undefined ? Array.from({ length: count }, (_, i) => round(f, i + 1)) : options.rounds;
...
test('exception accepts exactly two recorded visual failures', () => {
  const f = exceptionFixture(2);
  assert.equal(main([f.htmlRel], f.root).code, 0);
});

test('exception rejects more than two recorded visual failures', () => {
...
test('infrastructure failure counts, requires evidence gap, and has no screenshot/review', () => {
  const f = exceptionFixture(2, { attempts: [
...
test('fewer than two recorded failures is rejected even with repair records', () => {
  const f = exceptionFixture(1, { rounds: [] });
  assert.equal(main([f.htmlRel], f.root).code, 1);
});

test('missing or malformed attempt records are rejected', () => {
  const f = exceptionFixture(2, { attempts: [attempt('a'), {}] });
  assert.equal(main([f.htmlRel], f.root).code, 1);
});
...
test('non-visual theme gate remains blocking under exception', () => {
  const f = exceptionFixture(2, { theme_match: false });
  assert.equal(main([f.htmlRel], f.root).code, 1);
});
