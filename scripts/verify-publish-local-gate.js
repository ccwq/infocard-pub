 ...
 * This command never builds, stages, commits, pushes, or removes worktrees.
 */
...
function verifyIndexes(bundle, root) {
  ...
  if (!fs.existsSync(indexYaml)) {
    ...
    try {
      ...
      card = Array.isArray(parsed && parsed.cards) ? parsed.cards.find((item) => item && item.slug === bundle.slug) : null;
...
function verifyClean(root) {
  ...
  return status.trim() === '' ? [] : [error('git.status', 'worktree is dirty; cleanup is blocked and force removal is forbidden')];
}
...
function main(argv, cwd = process.cwd()) {
  ...
  try {
    const bundle = loadBundle(parsed.bundlePath);
    const bundleResult = validateBundle(bundle);
    const errors = [...bundleResult.errors, ...verifyWorktree(bundle, cwd)];
    const root = bundle.repository && typeof bundle.repository.root === 'string' ? path.resolve(bundle.repository.root) : cwd;
    if (bundleResult.valid && errors.length === 0) {
      errors.push(...verifyMeta(bundle, root));
      ...
      if (parsed.phase === 'postbuild' || parsed.phase === 'pre-cdn') errors.push(...verifyIndexes(bundle, root));
      if (parsed.phase === 'cleanup') errors.push(...verifyClean(root));
    }
...
module.exports = { main, parseStrictSingleDocumentMeta, verifyMeta, verifyIndexes, verifyWorktree };
