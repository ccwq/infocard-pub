...
顺序是硬门禁：**`.docs` authoring → manifest promotion → 本地渲染 → 桌面与移动截图（统一经由 `web-capture`，使用 `agent-browser --cdp 9222`）→ critical/major/minor 结论 → `npm run verify:visual-gate -- docs/<slug>.html` → build/verify → commit → push → 公网重新截图复核**。
