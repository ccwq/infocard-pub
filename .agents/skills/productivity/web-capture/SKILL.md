...
- **Preferred capture engine:** `agent-browser` attached to the endpoint supplied by the runtime
...
1. `agent-browser tab list` to find the stable tab id.
2. `agent-browser tab <id>` to focus the target page.
3. `agent-browser get url` and `get title` to confirm identity.
4. `agent-browser set viewport <w> <h>` using a preset or caller-supplied size.
5. `agent-browser eval` to read geometry when responsive correctness matters.
6. `agent-browser screenshot <path>` to write the PNG.
...
agent-browser tab t3
agent-browser set viewport 1280 900
agent-browser screenshot /tmp/visual/desktop.png

agent-browser set viewport 390 844
agent-browser eval 'JSON.stringify({innerWidth:innerWidth,scrollWidth:document.documentElement.scrollWidth,clientWidth:document.documentElement.clientWidth})'
agent-browser screenshot /tmp/visual/mobile.png
```
