...
Canonical local commands, run from the primary repository checkout after promotion:
...
The Publisher operates in the primary repository checkout. Inspect the working tree and preserve unrelated user changes. Copy only manifest-declared source files from `.docs/<card>/` to exact formal targets; do not use unrestricted directory copies or `git add -A`.
